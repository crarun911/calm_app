...
class _AudioPlayerScreenState extends State<AudioPlayerScreen>
  ...
  void _onSessionComplete() async {
    _clearSleepTimer();
    _audioPlayer.setVolume(1.0);
    // Pass category based on session type
    final category = widget.session.category.toLowerCase().contains('sleep')
        ? 'sleep'
        : widget.session.category.toLowerCase().contains('breath')
            ? 'breathing'
            : 'meditation';
    await context.read<AppState>().completeSession(category: category);
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        color:
                            _isFading ? AppTheme.rose : AppTheme.lavenderLight,
                        fontSize: 12,
                        fontWeight: FontWeight.w500,
                      ),
                    ),
                  ...
                  builder: (_, __) => Stack(
                    ...
                    children: [
                      ...[140.0, 120.0, 100.0].asMap().entries.map((e) {
                        final val =
                            (_pulseController.value + e.key * 0.3) % 1.0;
                        return Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                              color:
                                  accent.withOpacity(0.08 + (1 - val) * 0.05),
                              width: 1,
                            ),
                          ),
                        );
                ...
                children: [
                  ...
                  GestureDetector(
                    ...
                    child: const Column(children: [
                      ...
                      SizedBox(height: 4),
                      Text('Done',
                          style: TextStyle(color: AppTheme.sage, fontSize: 10)),
                    ]),
                  ),
                ],
              ),
